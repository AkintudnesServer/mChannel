...
public class mChannelAPI {
    ...
    public String getPlayersDefaultChannel(Player player) {
        for (Entry<String, Object> entry : plugin.channelNodeList.entrySet()) {
            if (mChat.API.checkPermissions(player, "mchannel.default."
                    + entry.getKey().replace(".default", ""), false))
                return entry.getKey().replace(".default", "");
        }
    ...
    public String getChannelType(String channelName) {
        if (isChannelReal(channelName)) {
            if (plugin.channelNodeList.get(channelName + ".type").toString() != null)
                return plugin.channelNodeList.get(channelName + ".type")
                        .toString();
        }
        return "";
    }
    ...
    public String getChannelPrefix(String channelName) {
        if (isChannelReal(channelName)) {
            if (plugin.channelNodeList.get(channelName + ".prefix").toString() != null)
                return plugin.channelNodeList.get(channelName + ".prefix")
                        .toString();
        }
        return "";
    }
    ...
    public String getChannelSuffix(String channelName) {
        if (isChannelReal(channelName)) {
            if (plugin.channelNodeList.get(channelName + ".suffix").toString() != null)
                return plugin.channelNodeList.get(channelName + ".suffix")
                        .toString();
        }
        return "";
    }
    ...
    public Integer getChannelDistance(String channelName) {
        if (isChannelReal(channelName)) {
            if (plugin.channelNodeList.get(channelName + ".distance")
                    .toString() != null)
                return (Integer) plugin.channelNodeList.get(channelName
                        + ".distance");
        }
        return 0;
    }
    ...
    public String getChannelPassword(String channelName) {
        if (isChannelReal(channelName)) {
            if (plugin.channelNodeList.get(channelName + ".password")
                    .toString() != null)
                return plugin.channelNodeList.get(channelName + ".password")
                        .toString();
        }
        return "";
    }
    ...
    public Boolean isChannelGlobalDefault(String channelName) {
        return isChannelReal(channelName)
                && (plugin.channelNodeList.get(channelName + ".default")
                        .equals(true));
    }
    ...
    public String getPlayersChannel(Player player) {
        if (plugin.playersChannel.get(player) == null) {
            player.performCommand("mchannel join "
    ...
    public void setPlayersChannel(Player player, String channelName) {
        if (player != null) {
            if (isChannelReal(channelName)) {
                ...
                if (plugin.channelLeaves.containsKey(channelName)
                        && plugin.channelLeaves.get(channelName).contains(
                                player)) {
                    plugin.channelLeaves.get(channelName).remove(player);
                }
            }
        }
    }
    ...
    public void removePlayerFromChannel(Player player, String channel) {
        if (player != null) {
            if (!isChannelReal(channel))
                return;
            if (plugin.mAPI.getPlayersChannel(player).equals(channel))
                removePlayersChannel(player);
            if (!plugin.channelLeaves.containsKey(channel))
                plugin.channelLeaves.put(channel, new HashSet<Player>());
            plugin.channelLeaves.get(channel).add(player);
        }
    }
    ...
    public void moveAllInChannel(String oldChannelName, String newChannelName) {
        if (isChannelReal(oldChannelName)) {
            if (plugin.playersChannel != null) {
                for (Entry<Player, String> entry : plugin.playersChannel
                        .entrySet()) {
                    if (entry.getValue().equals(oldChannelName)) {
                        ...
                        pEntry.sendMessage("Channel has been changed from "
                                + oldChannelName + " to " + newChannelName
                                + ".");
                    }
                }
            }
    ...
    public void createChannel(String channelName, String channelType,
            String channelPrefix, String channelSuffix,
            Integer channelDistance, Boolean channelDefault) {
        if (!(isChannelReal(channelName))) {
            plugin.channel.load();
    ...
    public void editChannel(String oldChannelName, String newChannelName,
            String channelType, String channelPrefix, String channelSuffix,
            Integer channelDistance, Boolean channelDefault) {
        createChannel(newChannelName, channelType, channelPrefix,
                channelSuffix, channelDistance, channelDefault);
    ...
    public void makeDefault(String channelName) {
        editChannel(channelName, channelName, getChannelType(channelName),
                getChannelPrefix(channelName), getChannelSuffix(channelName),
                getChannelDistance(channelName), true);
    }
}
